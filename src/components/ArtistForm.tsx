...
export default function ArtistForm({ initialData, onSubmit, onCancel }: ArtistFormProps) {
  const [name, setName] = useState(initialData?.name || "");
  const [email, setEmail] = useState(initialData?.email || "");
  const [status, setStatus] = useState<Artist["status"]>(initialData?.status || "actif");
    ...
    <form onSubmit={handleSubmit} className="space-y-4 text-white font-['PT_Sans']">
      ...
      <div>
        ...
        <select
          id="status"
          value={status}
          onChange={(e) => setStatus(e.target.value as Artist["status"])}
          className="w-full px-3 py-2 rounded bg-[#1A1B1F] border border-gray-600 focus:border-pink-500"
        >
